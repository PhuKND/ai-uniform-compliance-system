...
from app.services.student_repository import (
...
from app.utils.response_utils import ApiError, success_response
...
def _threshold() -> float:
    return float(current_app.config["SIMILARITY_THRESHOLD"])
...
def _student_response(record: dict[str, Any]) -> dict[str, Any]:
...
def _best_student_sample_match(student_id: str, embedding) -> dict[str, Any]:
    ...
    return {
        ...
        "sample_count": _student_response(student)["sample_count"],
    ...
    return success_response(
        {
            ...
            "pose_linking": {
                "enabled_by_default": bool(current_app.config.get("FACE_USE_SELECTED_POSE_BY_DEFAULT", True)),
...
def identify_image_for_selected_pose(image_path: Path, pose_result: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": "Selected face matched pose and identity threshold"
        if identity
...
def verify_image_for_selected_pose(
    ...
    if not pose_face_match.get("matched"):
        ...
        response.update(
            {
                "verified": False,
                "student": _student_response(_repository().get_student(student_id)),
                "sample_count": 0,
                "loaded_sample_count": len(_repository().get_loaded_samples(student_id)),
                "similarity": None,
                "threshold": _threshold(),
                "pose_face_match": {
                    "candidates": pose_face_match.get("candidates", []),
    ...
    selected_detection = pose_face_match["selected_detection"]
    try:
        match = _best_student_sample_match(student_id, selected_detection.embedding)
    ...
    return {
        ...
        "face_matched_to_selected_pose": True,
        "identity": student_id if match["matched"] else None,
        ...
        "image_saved_as": str(image_path),
        "reason": "Selected face matched pose and requested student"
        if match["matched"]
        else "Selected face matched pose but requested student did not match",
    }


@face_bp.post("/identify")
def identify_faces():
    """Identify every face found in an uploaded image."""

    image_path = _save_request_image("identify")
    use_selected_pose = _parse_bool(
        request.form.get("use_selected_pose"),
        default=bool(current_app.config.get("FACE_USE_SELECTED_POSE_BY_DEFAULT", True)),
    )
    if use_selected_pose:
        return success_response(
...
@face_bp.post("/verify")
def verify_student():
    """Verify that an uploaded face matches the requested student."""

    raw_student_id = _required_form_value("student_id")
    try:
        student_id = normalize_student_id(raw_student_id)
        _repository().get_student(student_id)
        if not _repository().get_loaded_samples(student_id):
            raise EmbeddingStorageError(f"No valid embeddings loaded for student_id '{student_id}'")
    except InvalidStudentIdError as exc:
        raise ApiError(str(exc), 400, "INVALID_STUDENT_ID") from exc
    except StudentNotFoundError as exc:
    ...
    if use_selected_pose:
        return success_response(
            verify_image_for_selected_pose(student_id, image_path),
    ...
    try:
        match = _best_student_sample_match(student_id, detection.embedding)
    except EmbeddingStorageError as exc:
        raise ApiError(str(exc), 500, "CORRUPTED_STORED_EMBEDDING") from exc
